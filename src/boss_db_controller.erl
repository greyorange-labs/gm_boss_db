...
handle_call(
    {find, Type, Conditions, Max, Skip, Sort, SortOrder, Include} = Cmd,
    From,
...
handle_call(
    {find, Type, Conditions, Max, Skip, Sort, SortOrder, _},
    _From,
    ...
    try connections_for_adapter(Adapter, Options) of
        {ok, {ReadConn, WriteConn}} ->
            ...
            {noreply, #state{
                ...
                connection_delay = 1,
                adapter = Adapter,
                read_connection = ReadConn,
                write_connection = WriteConn,
                shards = lists:reverse(Shards),
        ...
        Failure ->
            lager:info("Connection Error: ~p", [Failure]),
            reconnect_no_reply(Options, State, Adapter, CacheEnable, CacheTTL, CachePrefix)
    catch
        Error ->
            lager:info("Connection Excecption: ~p", [Error]),
            reconnect_no_reply(Options, State, Adapter, CacheEnable, CacheTTL, CachePrefix)
    end;
...
handle_info(stop, State) ->
    {stop, shutdown, State};
handle_info({'EXIT', _From, 'normal'}, State) when
    State#state.adapter =:= boss_db_adapter_mongodb
->
...
find_list(Type, Include, Cmd, From, Prefix, State, Key) ->
    ...
    case is_list(Res) of
        true ->
            ...
            IncludedRecords = find_list_records(
                Include,
                From,
                State,
                Res,
...
find_list_records(Include, From, State, Res, BelongsToTypes) ->
    lists:foldl(
        fun({RelationshipName, InnerInclude}, Acc) ->
            RelType = proplists:get_value(RelationshipName, BelongsToTypes),
            RecordList = lookup_rel_records(
                From,
                State,
                Res,
                RelationshipName,
...
lookup_rel_records(
    ...
    handle_call(
        {find, RelationshipName, [{'id', 'in', IdList}], all, 0, id, ascending, InnerInclude},
        From,
        State
    ).
...
reconnect_no_reply(Options, State, Adapter, CacheEnable, CacheTTL, CachePrefix) ->
    ...
    {noreply, #state{
        ...
        connection_retry_timer = Tref,
        adapter = Adapter,
        read_connection = undefined,
        write_connection = undefined,
        options = Options,
        cache_enable = CacheEnable,
        cache_ttl = CacheTTL,
        cache_prefix = CachePrefix
...
make_shards(Options, Adapter) ->
    lists:foldr(
        fun(ShardOptions, {ShardAcc, ModelDictAcc}) ->
            case proplists:get_value(db_shard_models, ShardOptions, []) of
                ...
                Models ->
                    ...
                    {ok, {ShardRead, ShardWrite}} = connections_for_adapter(
                        ShardAdapter, MergedOptions
                    ),
        ...
        {[], dict:new()},
        proplists:get_value(shards, Options, [])
    ).
